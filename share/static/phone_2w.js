...
let offer = async () => {
    ...
    console.log(off)
    localStorage.setItem('connection' ,  JSON.stringify({'In_Connect' : document.getElementById('to_user').value}))
...
let createAnswer = async (ID, offer) => {
    ...
    localStorage.setItem('connection' , JSON.stringify({'In_Connect' : ID}))
}
...
let video_share = async () => {
        localStream = await navigator.mediaDevices.getUserMedia({video:true, audio:true})
        document.getElementById('user1').srcObject = localStream
        localStorage.setItem('device_access' , JSON.stringify({'Camera' : 'Activated' , 'Audio' : 'Activated'}))
